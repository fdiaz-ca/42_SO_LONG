...
/*   Updated: 2022/11/28 15:12:30 by fdiaz-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* valids elements on it. Get position of player, door and baddies.          */
static int	ft_check_map(t_map *map, int x, int y)
{
	while (map->grid[y])
	{
		...
		else
		{
			...
			while (map->grid[y][x])
			{
				if (map->grid[y][x] == 'C' || map->grid[y][x] == 'B')
					ft_get_cb(map, x, y);
				else if (map->grid[y][x] == 'E' && map->door.x == NOT_YET)
					map->door = ft_get_pos(x, y);
				else if (map->grid[y][x] == 'P' && map->player.x == NOT_YET)
					map->player = ft_get_pos(x, y);
...
static void	ft_get_grid(t_map *map, int fd)
{
	...
	if (ft_check_map(map, 0, 0) == WRONG
		|| map->n_collec < 1 || map->door.x == NOT_YET
